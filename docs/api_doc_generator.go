...
func generateCollectionResourcePages() error {
	...
	apiNavBarName := "kulado-api-" + *apiVersion + "-sidebar-" + *version + ".html"
	...
	for fieldName, field := range schema.ResourceFields {
		...
		if includeAPIField {
			...
			if referenceRegexp.MatchString(field.Type) {
				...
				//[type]({{site.baseurl}}/kulado/{{page.version}}/{{page.lang}}/api/api-resources/type/)
				...
				apiField.TypeURL = "[" + field.Type + "]({{site.baseurl}}/kulado/{{page.version}}/{{page.lang}}/api/{{page.apiVersion}}/api-resources/" + field.Type + "/)"
			}
...
func getRefTypeURL(input string) string {
	return referenceRegexp.ReplaceAllString(input, "[$1]({{site.baseurl}}/kulado/{{page.version}}/{{page.lang}}/api/{{page.apiVersion}}/api-resources/$1/)")
}
...
func getTypeURL(typeInput string) string {
	...
	if _, isResourceType := schemaMap[resourceName]; isResourceType {
		urlResourceName := "[" + resourceName + "]({{site.baseurl}}/kulado/{{page.version}}/{{page.lang}}/api/{{page.apiVersion}}/api-resources/" + resourceName + "/)"
