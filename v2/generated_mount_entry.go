...
func newMountEntryClient(kuladoClient *KuladoClient) *MountEntryClient {
...
func (c *MountEntryClient) Create(container *MountEntry) (*MountEntry, error) {
	resp := &MountEntry{}
	err := c.kuladoClient.doCreate(MOUNT_ENTRY_TYPE, container, resp)
	return resp, err
}

func (c *MountEntryClient) Update(existing *MountEntry, updates interface{}) (*MountEntry, error) {
	resp := &MountEntry{}
	err := c.kuladoClient.doUpdate(MOUNT_ENTRY_TYPE, &existing.Resource, updates, resp)
	return resp, err
}

func (c *MountEntryClient) List(opts *ListOpts) (*MountEntryCollection, error) {
	resp := &MountEntryCollection{}
	err := c.kuladoClient.doList(MOUNT_ENTRY_TYPE, opts, resp)
	resp.client = c
	return resp, err
}
...
func (c *MountEntryClient) ById(id string) (*MountEntry, error) {
	resp := &MountEntry{}
	err := c.kuladoClient.doById(MOUNT_ENTRY_TYPE, id, resp)
...
func (c *MountEntryClient) Delete(container *MountEntry) error {
	return c.kuladoClient.doResourceDelete(MOUNT_ENTRY_TYPE, &container.Resource)
}
