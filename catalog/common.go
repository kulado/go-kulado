...
func (kuladoClient *KuladoBaseClientImpl) SetCustomHeaders(headers map[string]string) {
	kuladoClient.customHeaders = headers
}
...
func (kuladoClient *KuladoBaseClientImpl) setupRequest(req *http.Request) {
	...
	for k, v := range kuladoClient.customHeaders {
		req.Header.Set(k, v)
	}
}
...
func (kuladoClient *KuladoBaseClientImpl) Websocket(url string, headers map[string][]string) (*websocket.Conn, *http.Response, error) {
	...
	if kuladoClient.Opts != nil {
		s := kuladoClient.Opts.AccessKey + ":" + kuladoClient.Opts.SecretKey
