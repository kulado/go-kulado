...
func newPortRuleClient(kuladoClient *KuladoClient) *PortRuleClient {
...
func (c *PortRuleClient) Create(container *PortRule) (*PortRule, error) {
	resp := &PortRule{}
	err := c.kuladoClient.doCreate(PORT_RULE_TYPE, container, resp)
	return resp, err
}

func (c *PortRuleClient) Update(existing *PortRule, updates interface{}) (*PortRule, error) {
	resp := &PortRule{}
	err := c.kuladoClient.doUpdate(PORT_RULE_TYPE, &existing.Resource, updates, resp)
	return resp, err
}

func (c *PortRuleClient) List(opts *ListOpts) (*PortRuleCollection, error) {
	resp := &PortRuleCollection{}
	err := c.kuladoClient.doList(PORT_RULE_TYPE, opts, resp)
	resp.client = c
	return resp, err
}
...
func (c *PortRuleClient) ById(id string) (*PortRule, error) {
	resp := &PortRule{}
	err := c.kuladoClient.doById(PORT_RULE_TYPE, id, resp)
...
func (c *PortRuleClient) Delete(container *PortRule) error {
	return c.kuladoClient.doResourceDelete(PORT_RULE_TYPE, &container.Resource)
}
